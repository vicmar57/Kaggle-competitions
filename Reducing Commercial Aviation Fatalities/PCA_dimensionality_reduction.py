...
#TODO: 
test_df.drop(['experiment','seat'], axis=1, inplace=True)
train_df.drop(['experiment','seat'], axis=1, inplace=True)
...
X_train = sc.transform(x_train_df) 

pca = PCA(n_components=2)  
train_df = pca.fit_transform(x_train_df)  
...
train_labels = train_df['event']
explained_variance = pca.explained_variance_ratio_  


plot = pd.DataFrame(data=X_train, columns=['pc1','pc2'])  # 1st row as the column names
plot = pd.concat([plot,y], axis = 0)
plot['0'] = y
plot.drop([0] , axis=1 ,inplace=True) #X
plot['event'] = y
stop = timeit.default_timer()
print('\nRunTime:', stop - start, 'sec')


plot = sns.pairplot(x_vars=['pc1'], y_vars=['pc2'], data=plot, hue="event", size = 10)
plot.fig.suptitle("pc1 vs pc2")
...
#sub.to_csv("Baggingdepth2PCAexplain90per_noExpnoSeat.csv", index=False)
##%%
#
#
## -- coding: utf-8 --
